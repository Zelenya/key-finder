...
use crate::domain::shortcut_norm::{normalize_manual_shortcut, render_canonical_shortcut};
...
impl SqliteShortcutsRepository {
    pub(crate) fn add_shortcut(
        ...
        let shortcut = normalize_manual_shortcut(shortcut);
...
mod tests {
    ...
    #[test]
    fn add_shortcut_normalizes_manual_punctuation_keys() {
        let (_dir, apps, shortcuts) = init_apps_and_shortcuts();

        let app_id = apps.create_custom_app("Foo", &[]).expect("create app");

        shortcuts.add_shortcut(app_id, "cmd+,", "Open preferences").expect("manual shortcut");
        shortcuts.add_shortcut(app_id, "ctrl+`", "Toggle terminal").expect("manual shortcut");
        shortcuts.add_shortcut(app_id, "cmd+/", "Comment line").expect("manual shortcut");
        shortcuts.add_shortcut(app_id, "cmd+[", "Go back").expect("manual shortcut");
        shortcuts.add_shortcut(app_id, "cmd+left", "Move left").expect("manual shortcut");
        shortcuts.add_shortcut(app_id, "cmd+K, ->", "Focus next pane").expect("manual shortcut");

        let list = shortcuts.list_shortcuts(app_id, true).expect("list shortcuts");
        assert_eq!(list.len(), 6);
        assert!(
            list.iter()
                .any(|shortcut| shortcut.shortcut_display == "⌘ ,"
                    && shortcut.description == "Open preferences")
        );
        assert!(list
            .iter()
            .any(|shortcut| shortcut.shortcut_display == "⌃ `" && shortcut.description == "Toggle terminal"));
        assert!(list
            .iter()
            .any(|shortcut| shortcut.shortcut_display == "⌘ /" && shortcut.description == "Comment line"));
        assert!(list
            .iter()
            .any(|shortcut| shortcut.shortcut_display == "⌘ [" && shortcut.description == "Go back"));
        assert!(list
            .iter()
            .any(|shortcut| shortcut.shortcut_display == "⌘ ←" && shortcut.description == "Move left"));
        assert!(list.iter().any(
            |shortcut| shortcut.shortcut_display == "⌘ K, →" && shortcut.description == "Focus next pane"
        ));
    }
