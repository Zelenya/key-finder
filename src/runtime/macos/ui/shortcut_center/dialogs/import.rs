...
fn initial_dialog_state(
    ...
    let Ok(Some(path)) = import_sources::preferred_idea_keymap_file() else {
        return (None, default_state, default_status, default_detail);
    };
...
mod tests {
    ...
    use super::{initial_dialog_state, resolved_dialog_tag, TAG_MODE_CHANGED};
    ...
    fn prefill_state_uses_discovered_idea_keymap() {
        ...
        let state = initial_dialog_state(&spec, &target, None);
