...
fn build_new_shortcut_window(
    ...
    let intro = NSTextField::labelWithString(
        &NSString::from_str("Enter the shortcut and what it does in one step."),
        mtm,
    );
    intro.setFrame(NSRect::new(NSPoint::new(20.0, 182.0), NSSize::new(660.0, 20.0)));
    content.addSubview(&intro);

    let shortcut_field = add_labeled_text_field(&content, mtm, "Shortcut keys", 116.0, "For example: ⌘B");
