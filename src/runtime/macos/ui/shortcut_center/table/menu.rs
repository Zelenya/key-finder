...
fn menu_item(
    ...
    // SAFETY: `mtm` proves main-thread AppKit access, and this constructs a
    // fresh menu item with a selector implemented by `ShortcutCenterBridge`.
    let item = unsafe {
    ...
    // SAFETY: the bridge outlives the context menu items and exposes
    // `handleMenuAction:` as their target-action entry point.
    unsafe {
